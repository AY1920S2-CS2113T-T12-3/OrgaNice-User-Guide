...
Expected outcome:
The application would be display the list of commands supported.
A response similar to the following one would appear:
...
`<Time to finish task in days>` field isn't the deadline but rather the actual physical time it would take to finish the task.
